...
namespace LC
{
	partial class LexContext
	{
		...
		/// <summary>
		/// Attempts to read a DOS style string into the capture buffer
		/// </summary>
		/// <returns>True if a valid string was read, otherwise false</returns>
		public bool TryReadDosString()
		{
			EnsureStarted();
			if ('\"' != Current)
				return false;
		...
		/// <summary>
		/// Attempts to skip a DOS style string literal without capturing
		/// </summary>
		/// <returns>True if a literal was found and skipped, otherwise false</returns>
		public bool TrySkipDosString()
		{
			EnsureStarted();
			if ('\"' != Current)
				return false;
